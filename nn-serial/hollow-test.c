...
void generate_synthetic_data(float *data, float *targets, int num_samples) {
  for (int i = 0; i < num_samples; i++) {
    // Randomly decide if this sample will be center-filled or surround-filled
    int is_center = rand() % 2;
    targets[i * NUM_CLASSES + 0] = is_center ? 1.0f : 0.0f;
    targets[i * NUM_CLASSES + 1] = is_center ? 0.0f : 1.0f;

    // Generate image
    for (int y = 0; y < IMG_SIZE; y++) {
      for (int x = 0; x < IMG_SIZE; x++) {
        int pos = i * IMG_SIZE * IMG_SIZE + y * IMG_SIZE + x;
        int dist_x = abs(x - IMG_SIZE / 2);
        int dist_y = abs(y - IMG_SIZE / 2);
        int is_border = (dist_x >= IMG_SIZE / 4 || dist_y >= IMG_SIZE / 4);

        if (is_center) {
          // Content in center (central quarter of image), empty surround
          data[pos] = !is_border ? (rand() % 100) / 100.0f : 0.0f;
        } else {
          // Content in surround (outside central quarter), empty center
          data[pos] = is_border ? (rand() % 100) / 100.0f : 0.0f;
        }
      }
    }
...
int main() {
  ...
  float *errors = calloc(errors_Size, sizeof(float));
  ...
  for (int i = 0; i < 5; i++) {
    ...
    printf("\nTest %d: Predicted %s, Actual %s\n", i + 1,
           pred == 0 ? "CENTER" : "SURROUND",
           true_label == 0 ? "CENTER" : "SURROUND");
